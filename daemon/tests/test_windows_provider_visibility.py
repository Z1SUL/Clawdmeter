...
def _stub_paths(tmp_path, monkeypatch):
    """Point both provider path lookups at scratch files, neither of which
    exist yet — tests create them individually to flip a provider on."""
...
def test_all_absent_reports_all_disabled(_stub_paths):
    assert d._providers_enabled_now() == (False, False)


def test_only_claude_present(_stub_paths):
    claude_path, _ = _stub_paths
    claude_path.write_text("{}")
    assert d._providers_enabled_now() == (True, False)


def test_only_codex_present(_stub_paths):
    _, codex_path = _stub_paths
    codex_path.write_text("{}")
    assert d._providers_enabled_now() == (False, True)


def test_all_present(_stub_paths):
    claude_path, codex_path = _stub_paths
    claude_path.write_text("{}")
    codex_path.write_text("{}")
    assert d._providers_enabled_now() == (True, True)
...
def test_tick_sends_on_first_call(_stub_paths):
    ...
    assert session.written == [
        {"type": "providers", "claude": True, "codex": False}
    ]
...
def test_tick_resends_when_set_changes(_stub_paths):
    ...
    assert session.written[1] == {
        "type": "providers", "claude": True, "codex": True
    }
...
def test_tick_retries_after_a_failed_write(_stub_paths):
    ...
    _run(d.providers_enabled_tick(session))
    assert d._last_sent_providers_enabled == (True, False)
